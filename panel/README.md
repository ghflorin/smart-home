...
a percentage. Here the device speaks for itself, which is what you want when the
question is "why is it behaving like that".

- **It costs no radio traffic.** The whole thing is a read of matter-server's
  cache, so it works on a sleeping battery device — the one you most often want
  to interrogate, and the one a direct read is least likely to reach.
- **A refusal is shown as a refusal.** A read that failed is cached as an object
  describing the failure, not as a value; rendering that as one would put
  `UnsupportedAttribute` in a table of readings as though the device had said it.
  Unsupported attributes are marked, and a cluster whose every attribute refused
  is dropped — it is the residue of somebody having asked, not a capability.
- **The names come from the SDK.** `panel/matter_names.json` is generated by
  `scripts/gen-matter-names.py`: 140 clusters and 1854 attributes. Hand-writing
  that table would be wrong within a release. Without the file the inspector
  still works and shows numbers.
- The six global attributes every cluster carries are shown, because "everything"
  means everything, but recessed — they are identical house-wide.
